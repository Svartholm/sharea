<?php
	...
	class Folder extends \lib\Record
	{
		const NO_PARENT = -1;
		...
		{
			if(is_object($permissions) && get_class($permissions) == "lib\Permission")
				$this->permissions = $permissions;
			else
				$this->permissions = new \lib\Permission($permissions);
		}
		...
		public function isValid($passthrough = false)
		{
			...
			if(!\lib\Regex::isFolderName($this->name))
				{
			...
			if(!\lib\Regex::isPermissionSerial($this->permissions->getSerial()))
				{
		...
		public function getFolders(\lib\models\FolderManager $manager)
		{
			return $manager->getFoldersOf($this);
		}
		
		public function getAllChildren(\lib\models\FolderManager $manager)
		{
			return $manager->getAllChildrenOf($this);
		}
		
		public function getFiles(\lib\models\FolderManager $manager)
		{
			return $manager->getFilesOf($this);
		}
		
		public function hasChildren(\lib\models\FolderManager $manager)
		{
