...
function displayFoldersList(data) {
    ...
    a.setAttribute('data-content', 'Nom : <b>Racine</b>');
    a.setAttribute('title', 'Propriétés du dossier');

    a.setAttribute("onclick", "backFolderList()");
    a.innerHTML = '<td><img src="/images/folder_home.png" alt="racine"></td><td>Retour</td><td>--</td><td>Privé</td><td class="options" style="visibility:hidden"><li class="dropdown" data-dropdown="dropdown"><a data-toggle="dropdown" class="btn btn-small btn-primary dropdown-toggle options"><span class="caret"></span></a></td></tr>';
    ...
    if (data.files != null) {
        for (var i = 0; i < data.files.length; i++) {
            ...
            hash = '<li class="dropdown" data-dropdown="dropdown">' +
                ...
                '<li class="divider"></li><li><a href="#" onclick="supprimer(\'' + data.files[i].id + '\', \'' + data.files[i].name + '\')" data-controls-modal="supprimerfichier" data-backdrop="static"><i class="icon-remove"></i> Supprimer</a></li>' +
                '</ul>' +
                '</li>';
