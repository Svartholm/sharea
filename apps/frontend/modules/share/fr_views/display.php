...
<div class="container">
    <div id="top">
        <h2 class="title">ShareBox de <?php echo $pseudo; ?></h2>
        ...
        if($has_parent) {
            echo '<div class="fd">';
            echo '<a href="#" onclick="window.location.replace(document.referrer)"><img src="/images/folder_home.png" alt="home"><br> Retour</a>';
            echo '</div>';
        }
        $count = 1;
        foreach ($folders as $folder) {
            ...
            <div class="fd">
                ...
                <a href="/users/<?php echo $pseudo;?>/files/<?php echo $folder->id(); ?>" id="name<?php echo $count; ?>"
                   ...
                   data-content="Nom : <b><?php echo $name; ?></b><br>Visible par : <b><?php echo $permission; ?></b><br />Date de création : <b><?php echo $folder->date(); ?></b>"
...
// AFFICHAGE DES FICHIERS
        foreach ($files as $file) {
            ...
            <div class="fd">
                ...
                ?>
                id="name<?php echo $count; ?>" data-rel="popover" data-content="Nom :
                <b><?php echo htmlentities($name, ENT_QUOTES, 'UTF-8'); ?></b><br/>Visible par : <b>
                <?php
            ...
            </b><br/>Taille : <b><?php echo number_format($file->size() / 1024 / 1024, 2); ?> Mo</b><br/>Type :
                <b><?php echo $file->mimetype(); ?></b><br/>Date d'envoi : <b><?php echo $file->date(); ?></b>" title="Propriétés du
                fichier">
                ...
                <li class="dropdown" data-dropdown="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a href="/download/<? echo $file->id();?>" data-backdrop="static"><i class="icon-download"></i>
                            Télécharger</a></li>
                        <li><a href="#" onclick="importFile(<?php echo $file->id(); ?>)"><i class="icon-plus-sign"
                                                                                            style="opacity:0.8"></i> Importer le
                            fichier dans mes documents</a></li>
                        <li><a href="#" onclick="qrcode('<?php echo $file->id(); ?>', '<?php echo $file->name(); ?>')"
                               data-controls-modal="qrcodefichier" data-backdrop="static"><i class="icon-qrcode"></i> Lien de
                            téléchargement</a></li>
                    </ul>
                </li>
            </div>
        ...
        jQuery.ajax({
            ...
            success:function (data, textStatus, jqXHR) {
                if (check(data, true, false) !== false) {
                    alert("Le fichier a été importé dans votre espace personnel avec succès");
                }
            },
            error:function (jqXHR, textStatus, errorThrown) {
                alert("Erreur lors l'importation du fichier");
            }
        });
    }
</script>
